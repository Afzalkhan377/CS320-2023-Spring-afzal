...
fun list_averages(xs: real list): real list =
  let
    fun avg_helper(xs: real list, count: real, sum: real): real list =
      case xs of
        [] => []
      | x::xs => 
          let
            val new_sum =  sum + x  
            val new_count = count + 1.0
          in
            (new_sum / new_count) :: avg_helper(xs, new_count, new_sum)
          end
  in
    avg_helper(xs, 0.0, 0.0)
  end
(* ****** ****** *)
