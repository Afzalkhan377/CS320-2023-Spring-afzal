...
|
xlist_snoc(xs, x1) =>  if i0=xlist_size(xs) then x1 else xlist_subhelper(xs,i0)
|
xlist_append(xs, ys) => if i0< xlist_size(xs) then xlist_subhelper(xs,i0) else  xlist_subhelper(ys,i0-xlist_size(xs))
|
xlist_reverse(xs) =>xlist_subhelper(xs, xlist_size(xs)-i0-1)


fun xlist_sub(xs: 'a xlist, i0: int): 'a=
if i0<0 orelse i0>= xlist_size(xs) then raise XlistSubscript else
